...
- C# 버전
    ...
    - ...
    - C# 6.x - .NET Framework 4.6, .NET Core 1.x(리눅스)
...
- 호환성, 범용성
    ...
    - 2016년부터 리눅스 공식지원. macOS에서도 사용 가능. 오픈 소스 공표
    - 유니티 엔젠 기본언어, Xamarin(이후 MAUI로 변경)로 모바일 개발 가능
...
10. 새로 만든 컨트롤을 더블클릭
    - 이벤트 메서드가 자동 생성
    - 최초에 아무런 이벤트가 없을 때 기본 이벤트 메서드 발생
...
- 기본 문법 - C++, Java와 거의 동일. C++와의 차이점만 비교
    ...
    4. 분기문, 반복문 - [소스](./day02/Day02Study/SysntaxWinApp03/FrmMain.cs)
        - if문
        - switch문
        - for문
        - foreach는 컬렉션에서...
...
1. Debug - 디버깅시 필요한 기능 담은 클래스
    - Debug.WriteLline() : 디버그 > 창 > `출력`에 디버깅 로그 출력
    - 디버깅 시작으로 실행 시만 출력
2. 프로젝트 속성
...
### C# 문법
- 기본 문법
    1. 문자열 처리 - [소스](./day02/Day02Study/SysntaxWinApp04/FrmMain.cs)
        - 콘솔에서 입력되는 값 -> 모두 문자열
        - 윈앱에서 TextBox에서 입력되는 값 -> 모두 문자열

        <img src="./image/cs0004.png" witdh="600">
    2. 클래스와 객체, 메서드, 속성 - [소스](./day03/Day03Study/SysntaxWinApp1/Person.cs)
        - 객체를 생성하는 틀
        - 명사(멤버변수, 속성), 동사(멤버메서드)
        - Get/Set
    3. 접근제어자
    ...
    4. 상속, 다형성, 추상클래스, 인터페이스 - [소스](./day03/Day03Study/SysntaxWinApp02/Computer.cs)
...
- `RichTextBox` : 포맷팅이 가능한 텍스트박스
    ...
    - ScrollBars : 컨트롤 오른쪽, 하단에 스크롤바 생성
        - Both 보다는 ForcedBotn\h, ForcedVertical 사용
...
- 기본문법
    ...
    2. 예외처리
        ...
        https://github.com/user-attachments/assets/16afba1c-71fd-4f95-a471-a83bed7acb3e
...
- 기본 문법
    1. 델리게이트(대리자), 이벤트 - [소스](./day05/Day05Study/SyntaxWinApp01/FrmMain.cs)
        ...
        - 대리자는 실행할 메서드를 들고 있고, 대리자를 호출하면 대리자가 메서드 대신 호출해줌
        ...
        - 자주 쓰이는 이벤트
            ...
            - Button_Click - 생략
            - TextBox_KeyPress - 생략
            - ComboBox_SelectedInexChanged - 생략
...
- `BackgroundWorker` : 화면 뒷단에서 작업할 일중 스레드 처리가 되어야할 부분을 손쉽게 동작시켜주는 컨트롤(화면표시 X)
    ...
    - `RunWorkerCompleted` : 작업완료시 대부분 UI 완료 처리
...
- `TrackBar` : 사운드, 화면밝기등 최소, 최대를 스크롤로 조정하는 컨트롤
    - Trb~ 로 이름 사용
    - Minimum, Macimum : ProgressBar와 동일
...
- `MenuStrip` : 프로그램 메뉴관리
    - Mnu~ 로 이름시작. 사용하는 메뉴는 반드시 이름을 변경할 것 (예: 끝내기XMenustrip)와 같이 이름이 생성
...
- C# 기본
    ...
    - 모달창, 모달리스 창 : 창위에 다른 창을 띄울때 접근 권한 제어 형태
        - Modal : 부모창과 연관된 작업을 할때 사용. 모달창이 닫히지 않으면 부모창을 제어할 수 없음
        - Modaless : 부모창과 상관없이 동작. 사용 주의. 메인창을 언제나 닫을 수 있고, 메인창은 닫으면 모두 종료. 사용시 주의
...
- WPF 개요
    ...
    - 비트맵방식의 문제를 해결 -> 벡터방식(테스트). 속도 개선, 용량 절약
...
### ToyProject - ToDayDiary

#### 일기장 프로그램
- **ToDayDiary**는 개인의 하루를 기록할 수 있는 `일기장` 앱입니다.
    파일 저장 기능을 통해 언제 어디서든 손쉽게 일기를 작성하고 관리할 수 있습니다.

    - 기본 화면

        <img src="./image/기본화면.png" width="600">

    - 입력 화면

        <img src="./image/입력화면.png" width="600">

- 주요 기능
    - **캘린더로 날짜 선택**            
        - 캘린더를 통해 `오늘 날짜`를 확인할 수 있다.
        - 원하는 날짜를 선택시 `자동으로 일기장에 해당 날짜가 기입`된다.

        <img src="./image/날짜 선택.png" width="600">

    - **요일 자동 인식**
        - 선택한 날짜가 `평일`인지 `주말`인지 안내

        <img src="./image/주말표시.png" width="600">
        
    - **이미지 첨부**
        - 원하는 `이미지`를 불러와서 오늘 하루를 표현할 수 있는 이미지를 넣을 수 있다.
    
    - **일기 저장 및 불러오기**
        - 작성한 일기를 `저장`하고, 저장한 파일을 `불러와서` 읽을 수 있다.
